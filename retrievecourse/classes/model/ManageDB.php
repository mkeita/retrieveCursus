...
class ManageDB {
	...
	public function getContextid($courseid, $contextlevel = CONTEXT_COURSE) {
		global $DB;
		$data = $DB->get_record ( 'context', array (
				"instanceid" => $courseid,
	 ...
	 * 
	 * @param string $course
	 *        	Le shortname du cours dont on veut vérifier l'existence.
	...
	public function checkCourseExist($course) {
		...
		return $DB->record_exists ( 'course', array (
				"shortname" => $course 
		) );
	}
	 ...
	 * 
	 * @param string $course
	 *        	Id du cours dont on veut vérifier l'existence.
	 * @return vrai si le cour existe.
	 */
	...
	public function getCourseId($shortname) {
		global $DB;
		$data = $DB->get_record ( 'course', array (
				"shortname" => $shortname 
		), 'id' );
		return (($data != NULL) ? $data->id : NULL);
	}
	...
	public function checkUserExist($userid) {
		global $DB;
		$used = $DB->record_exists ( 'user', array (
				"id" => $userid 
		) );
		return $used;
	}
