...
class ManageRetrieveCourseCronDB extends ManageDB {
	...
	public function addCourse_cron($idCourse, $userid, $nextShortname) {
		...
		$dataobject = array (
				'courseid' => $idCourse,
				'user' => $userid,
				'shortname_course_new' => $nextShortname,
				'status' => RetrieveCourseConstante::STATUS_WAITING,
				'time_created' => time (),
				'tentative' => 0 
		);
	 ...
	 * 
	 * @param unknown $id
	 *        	L'id du tuple qu'il faut modifier.
	 * @param unknown $timeModified
	 *        	La nouvelle valeur de timeModified.
	...
	public function getCronTentative($id) {
		global $DB;
		$data = $DB->get_record ( 'retrievecourse_cron', array (
				"id" => $id 
		), 'tentative' );
	...
	public function getTimeModifiedCron($id) {
		...
		var_dump ( $result [$id]->time_modified );
		return ($result == NULL) ? NULL : $result [$id]->time_modified;
	}
	...
	/**
	 * Permet de vérifier que le cours passer en paramétre est bien celui executé par Cron
	 * 
	 * @return vrai si le cours entré en paramétre équivaut à celui qui est exécuté.
	 */
	public function isRunning($idCourse_old) {
		global $DB;
		$obj = $DB->get_records_sql ( 'SELECT id,courseid from mdl_retrievecourse_cron WHERE mdl_retrievecourse_cron.status = "execute"' );
		$courseid;
		foreach ( $obj as $val ) {
			$courseid = $val->courseid;
		}
		return ($obj == NULL) ? NULL : ($courseid == $idCourse_old);
	}
	...
	public function deleteCron($id) {
		global $DB;
		return $DB->delete_records ( 'retrievecourse_cron', array (
				"id" => $id 
		) );
	}
}