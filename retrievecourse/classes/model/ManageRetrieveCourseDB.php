...
class ManageRetrieveCourseDB extends ManageDB {
	...
	public function checkPluginUsed($id) {
		global $DB;
		$used = $DB->record_exists ( 'retrievecourse', array (
				"courseid_old" => $id 
		) );
		return $used;
	}
	...
	public function addCourse_retrievecourse($shortname_old, $shortname_new, $temp, $courseid_old, $flag_newcourse = 0, $flag_wait_cron = 0, $flag_use_cron = 0) {
		...
		if ($idCourse != null) {
			...
			$dataobject = array (
					'courseid_old' => $courseid_old,
					'courseid_new' => $idCourse,
					'shortname_course_old' => $shortname_old,
					'shortname_course_new' => $shortname_new,
					'user' => $USER->id,
					'annac' => $temp,
					'date' => time (),
					'flag_newcourse' => $flag_newcourse,
					'flag_use_cron' => $flag_use_cron,
					'flag_wait_cron_execute' => $flag_wait_cron 
			);
			$DB->insert_record ( 'retrievecourse', $dataobject );
		}
	}
	public function updateTimeStart($courseid, $time) {
		global $DB;
		$dataobject = array (
	 ...
	 */
	public function updateTimeModifiedCron($courseid, $timeModified) {
		global $DB;
		$dataobject = array (
				'courseid_old' => $id,
	...
	public function getRetrievecourseId($courseid_old) {
		global $DB;
		$data = $DB->get_record ( 'retrievecourse', array (
				"courseid_old" => $courseid_old 
		), 'id' );
	...
	public function courseNotUsedPugin($idCagtegorie = null) {
		...
		$result = $DB->get_records_sql ( 'SELECT mdl_course.id,mdl_course.shortname FROM mdl_course
				WHERE mdl_course.id NOT IN (SELECT mdl_retrievecourse.courseid_old FROM mdl_retrievecourse)' . $cond );
	...
	private function deleteOldRetrieve() {
		global $DB, $PAGE;
		$used = $DB->record_exists ( 'retrievecourse', array (
				"courseid_new" => $_SESSION ['idCourse'] 
		) );
		if ($used) {
